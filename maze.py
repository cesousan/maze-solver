...
class Maze:
    ...
    def _solve_r(self, i, j):
        ...
        def has_wall(i, j, direction):
            return getattr(self._cells[i][j], f"has_{direction}_wall")
        ...
        for x, y, direction in [
            ...
            if (
                is_in_bounds(i + x, j + y)
                and not is_visited(i + x, j + y)
                and not has_wall(i, j, direction)
            ):
                self._cells[i][j].draw_move(self._cells[i + x][j + y])
                if self._solve_r(i + x, j + y):
                    return True
                self._cells[i][j].draw_move(self._cells[i + x][j + y], undo=True)
